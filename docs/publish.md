...
- `bump_and_tag` generates AI release notes with `everydaydevopsio/castoff/castoff@v2` and inserts a `CHANGELOG.md` entry with `everydaydevopsio/castoff/changelog@v2`, committing the changelog alongside the version bump. `OPENAI_API_KEY` is required: the job fails fast when it is missing rather than tagging a release with no changelog entry. Set the `OPENAI_MODEL` repository variable to override the model.
