...
- Skills install into target-specific skill locations such as `.claude/skills/aws-health-review/SKILL.md`, `.gemini/rules/aws-live-health-review.md`, and `.cursor/rules/aws-weekly-security-review.mdc`
...
- TypeScript-only repositories use Husky with `lint-staged` or the repository formatter/linter.
- TypeScript-only Husky pre-commit hooks should use `lint-staged` or the repository formatter/linter for fast formatting checks for `.yaml` and `.yml` files.
- TypeScript-only Husky `.husky/pre-push` hooks should run the detected or canonical package-manager test command, with build or typecheck first when the repository convention requires it.
...
- Pre-commit repositories should declare the official Gitleaks hook in `.pre-commit-config.yaml` for secret detection and should not generate or call a repo-local no-secrets shell script.
...
- Keep heavier language-aware security checks in CI or review workflows unless the repository explicitly opts into running them from hooks: Go `govulncheck`, fuzzing, and `go test -race`; Python Bandit and `pip-audit`; JavaScript/TypeScript package-manager audits; Ansible `ansible-lint --profile=safety`; Terraform/IaC static analysis, policy checks, and cloud/runtime posture scans.
...
| Claude   | `.claude/rules/` | `.claude/skills/`   | agents use `<agent>.md` or `<language>-<agent>.md`; skills use `<skill>/SKILL.md` and are invoked as `/<skill>` |
