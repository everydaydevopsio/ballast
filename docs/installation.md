...
## Monorepo Setup: TypeScript + Python + Go + Ansible

For mixed-language monorepos, prefer the unified `ballast` wrapper from the repository root for TypeScript, Python, Go, and Ansible subprojects.
...
- Python, Go, and Ansible rules use `pre-commit` and should create or update `.pre-commit-config.yaml` files.
