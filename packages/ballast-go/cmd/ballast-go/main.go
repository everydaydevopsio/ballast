...
func renderGitHooksGuidance(language, hookMode string) string {
	secretHookGuidance := "- Add the official `gitleaks` pre-commit hook in `.pre-commit-config.yaml` for secret detection; do not generate or call a repo-local no-secrets shell script."
	switch language {
	case "typescript":
		if hookMode == "husky" {
			return strings.Join([]string{
